...
using NieblaMejorada.Core;


namespace NieblaMejorada.URP
{
    public class NieblaMejorada : ScriptableRendererFeature
    {
        [Serializable]
        public class NieblaMejoradaSettings
        {
        ...
        public NieblaMejoradaSettings m_Settings = new NieblaMejoradaSettings();
        ...
        private NieblaMejoradaVolumeComponent m_NieblaMejoradaVolume;
        public static int kMaxIterations = 16;

        public bool m_UseSMSS
        {
            get { return m_NieblaMejoradaVolume._UseSSMS.value; }
        }
...

        {
            ...
            // [Update 2.1] disable niebla mejorada by toggling off Post Processing in camera settings
            if (renderingData.cameraData.postProcessEnabled == false)
                return;

            m_NieblaMejoradaVolume = VolumeManager.instance.stack?.GetComponent<NieblaMejoradaVolumeComponent>();
            if (m_NieblaMejoradaVolume == null || !m_NieblaMejoradaVolume.IsActive())
                return;
            ...
            if (m_SMSSMaterial == null) m_SMSSMaterial = CoreUtils.CreateEngineMaterial(Shader.Find("Hidden/SSMS_URP"));
            ...
            if (renderingData.cameraData.cameraType == CameraType.Game)
            {
                ...
                var set = sceneCamera.GetComponent<NieblaMejoradaRenderers>();
        ...
        private void FogFactorMaterialProperties()
        {
            ...
            if (m_NieblaMejoradaVolume._UseDistanceFog.value)
            {
            ...
            if (m_NieblaMejoradaVolume._UseSkyboxHeightFog.value)
            {
            ...
            if (m_NieblaMejoradaVolume._UseHeightFog.value)
            {
            ...
            if (m_NieblaMejoradaVolume._UseNoise.value)
            {
            ...
            m_FogFactorMaterial.SetFloat("_FogIntensity", m_NieblaMejoradaVolume._FogIntensity.value);
            m_FogFactorMaterial.SetInt("_UseRadialDistance", m_NieblaMejoradaVolume._UseRadialDistance.value ? 1 : 0);

            switch (m_NieblaMejoradaVolume._FogType.value)
            {
                case FogMode.Linear:
            ...
            m_FogFactorMaterial.SetFloat("_DistanceFogOffset", m_NieblaMejoradaVolume._DistanceFogOffset.value);

            m_FogFactorMaterial.SetFloat("_SkyboxFogIntensity", m_NieblaMejoradaVolume._SkyboxFogIntensity.value);
            m_FogFactorMaterial.SetFloat("_SkyboxFogHardness", m_NieblaMejoradaVolume._SkyboxFogHardness.value);
            m_FogFactorMaterial.SetFloat("_SkyboxFogOffset", m_NieblaMejoradaVolume._SkyboxFogOffset.value);
            m_FogFactorMaterial.SetFloat("_SkyboxFill", m_NieblaMejoradaVolume._SkyboxFill.value);

            m_FogFactorMaterial.SetFloat("_HeightDensity", Mathf.Pow(m_NieblaMejoradaVolume._HeightDensity.value, 4));
            m_FogFactorMaterial.SetFloat("_Height", m_NieblaMejoradaVolume._Height.value);
            m_FogFactorMaterial.SetInt("_HeightFogTypeExp", ((int)m_NieblaMejoradaVolume._HeightFogType.value == 2 ? 1 : 0));

            m_FogFactorMaterial.SetFloat("_Scale1", m_NieblaMejoradaVolume._Scale1.value);
            m_FogFactorMaterial.SetFloat("_NoiseTimeScale1", m_NieblaMejoradaVolume._NoiseTimeScale1.value);
            m_FogFactorMaterial.SetFloat("_Lerp1", m_NieblaMejoradaVolume._Lerp1.value);
            m_FogFactorMaterial.SetFloat("_NoiseDistanceEnd", m_NieblaMejoradaVolume._NoiseDistanceEnd.value);
            m_FogFactorMaterial.SetFloat("_NoiseIntensity", m_NieblaMejoradaVolume._NoiseIntensity.value);
            m_FogFactorMaterial.SetFloat("_NoiseEndHardness", m_NieblaMejoradaVolume._NoiseEndHardness.value);

            m_FogFactorMaterial.SetVector("_NoiseSpeed1", m_NieblaMejoradaVolume._NoiseSpeed1.value);

            int useNoiseDistance = 0;
            int useNoiseHeight = 0;

            switch (m_NieblaMejoradaVolume._NoiseAffect.value)
            {
            ...
            if (m_NieblaMejoradaVolume._UseDistanceFog.value == false)
                useNoiseDistance = 0;

            if (m_NieblaMejoradaVolume._UseHeightFog.value == false)
                useNoiseHeight = 0;
            ...
            // Distance Fog Values
            Vector4 sceneParams;
            float diff = m_NieblaMejoradaVolume._SceneEnd.value - m_NieblaMejoradaVolume._SceneStart.value;
            float invDiff = Mathf.Abs(diff) > 0.0001f ? 1.0f / diff : 0.0f;
            sceneParams.x = m_NieblaMejoradaVolume._FogDensity.value * 1.2011224087f; // density / sqrt(ln(2)), used by Exp2 fog mode
            sceneParams.y = m_NieblaMejoradaVolume._FogDensity.value * 1.4426950408f; // density / ln(2), used by Exp fog mode
            sceneParams.z = -invDiff;
            sceneParams.w = m_NieblaMejoradaVolume._SceneEnd.value * invDiff;
        ...
        private void FogBlendMaterialProperties()
        {
            ...
            if (m_NieblaMejoradaVolume._UseSunLight.value)
            {
            ...
            if (m_NieblaMejoradaVolume._UseGradient.value)
            {
            ...
            m_FogBlendMaterial.SetColor("_SunColor", m_NieblaMejoradaVolume._SunColor.value);
            m_FogBlendMaterial.SetFloat("_SunPower", m_NieblaMejoradaVolume._SunPower.value);
            m_FogBlendMaterial.SetFloat("_SunIntensity", m_NieblaMejoradaVolume._SunIntensity.value);

            m_FogBlendMaterial.SetColor("_FogColor", m_NieblaMejoradaVolume._FogColor.value);

            m_FogBlendMaterial.SetFloat("_GradientStart", m_NieblaMejoradaVolume._GradientStart.value);
            m_FogBlendMaterial.SetFloat("_GradientEnd", m_NieblaMejoradaVolume._GradientEnd.value);

            // Custom texture lerping
            // TODO change to standard TextureParemeter after Unity adds Texture Interp (in 2023 not available yet)
            if (m_NieblaMejoradaVolume._GradientTexture.value != null)
            {
                m_FogBlendMaterial.SetFloat("_GradientLerp", m_NieblaMejoradaVolume._GradientTexture.LerpValue);
                m_FogBlendMaterial.SetTexture("_GradientTextureFrom", m_NieblaMejoradaVolume._GradientTexture.FromTexture);
                m_FogBlendMaterial.SetTexture("_GradientTexture", m_NieblaMejoradaVolume._GradientTexture.value);
            }

            m_FogBlendMaterial.SetFloat("_EnergyLoss", m_NieblaMejoradaVolume._EnergyLoss.value);
        }

        private int SSMSProperties(float tw, float th)
        {
            // determine the iteration count
            var logh = Mathf.Log(th, 2) + m_NieblaMejoradaVolume._Radius.value - 8;
            var logh_i = (int)logh;
            var iterations = Mathf.Clamp(logh_i, 1, kMaxIterations);

            // update the shader properties
            var lthresh = m_NieblaMejoradaVolume._Threshold.value;
            m_SMSSMaterial.SetFloat("_Threshold", lthresh);

            var knee = lthresh * m_NieblaMejoradaVolume._SoftKnee.value + 1e-5f;
            var curve = new Vector3(lthresh - knee, knee * 2, 0.25f / knee);
            m_SMSSMaterial.SetVector("_Curve", curve);

            var pfo = !m_NieblaMejoradaVolume._HighQuality.value && m_NieblaMejoradaVolume._AntiFlicker.value;
            m_SMSSMaterial.SetFloat("_PrefilterOffs", pfo ? -0.5f : 0.0f);

            m_SMSSMaterial.SetFloat("_SampleScale", 0.5f + logh - logh_i);
            m_SMSSMaterial.SetFloat("_Intensity", m_NieblaMejoradaVolume._Intensity.value);

            var fadeRampTexture = m_NieblaMejoradaVolume._FadeRamp.value;
            if (fadeRampTexture != null) m_SMSSMaterial.SetTexture("_FadeTex", fadeRampTexture);
            m_SMSSMaterial.SetFloat("_BlurWeight", m_NieblaMejoradaVolume._BlurWeight.value);
            m_SMSSMaterial.SetFloat("_Radius", m_NieblaMejoradaVolume._Radius.value);

            if (m_NieblaMejoradaVolume._AntiFlicker.value)
            {
            ...
            if (m_NieblaMejoradaVolume._HighQuality.value)
            {
    ...
    public class NieblaMejoradaPassData : ContextItem, IDisposable
    {
        ...
        public void RecordTemporaryBlit(RenderGraph renderGraph, ContextContainer frameData,bool useSMSS)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PassData>("Niebla Mejorada Temporary Blit", out var passData))
            {
                var resourceData = frameData.Get<UniversalResourceData>();
        ...
        class PassDataSMSS
        {
            ...
            public TextureHandle[] _blurBuffer1 = new TextureHandle[NieblaMejorada.kMaxIterations];
            public TextureHandle[] _blurBuffer2 = new TextureHandle[NieblaMejorada.kMaxIterations];
        }
    ...
    class CustomDepthPass : ScriptableRenderPass
    {
        ...
        {
            var blitTextureData = frameData.GetOrCreate<NieblaMejoradaPassData>();
